...
from userbot.utils import skyla_cmd, edit_delete, edit_or_reply
...
@skyla_cmd(pattern=r"(?:tikel|kang)\s?(.)?")
async def kang(args):
...
async def pussy(args):
    ...
    if message and message.media:
        ...
        return
    cmd = "/newvideo"
    packname = f"skyla_{userid}_temp_pack"
    response = urllib.request.urlopen(
        urllib.request.Request(f"http://t.me/addstickers/{packname}")
    )
    htmlstr = response.read().decode("utf8").split("\n")
...
@skyla_cmd(pattern=r"getsticker$")
